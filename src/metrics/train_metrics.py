...
class TrainLoss(nn.Module):
    ...
    def forward(self, masked_pred_X, masked_pred_E, pred_y, true_X, true_E, true_y, log: bool):
        mse_X = self.train_node_mse(masked_pred_X, true_X) if true_X.numel() > 0 else 0.0
        mse_E = self.train_edge_mse(masked_pred_E, true_E) if true_E.numel() > 0 else 0.0
    ...
    def log_epoch_metrics(self):
        ...
        to_log = {"train_epoch/x_CE": epoch_node_mse,
                  "train_epoch/E_CE": epoch_edge_mse,
                  "train_epoch/y_CE": epoch_y_mse}
        if wandb.run:
            wandb.log(to_log)
        return to_log
...
class TrainLossDiscrete(nn.Module):
    ...
    def __init__(self, lambda_train):
        ...
        #self.node_loss = NodeMSE()
        #self.edge_loss = EdgeMSE()
    ...
    def forward(self, masked_pred_X, masked_pred_E, pred_y, true_X, true_E, true_y, log: bool):
        ...
        #print('pred', flat_pred_X)
        #print('true', flat_true_X)
