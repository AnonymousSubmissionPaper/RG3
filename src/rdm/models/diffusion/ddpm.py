...
class DDPM(nn.Module):
    ...
    def reconstruct_input(self, x_start, t, model_output):
        # Assuming model_output is the predicted noise
        noise = model_output

        # Calculate the scaling factors using the cumulative products
        ...
        # Use the inverse of the q_sample operation to reconstruct x_start
        reconstructed_x_start = (x_start - sqrt_one_minus_alphas_cumprod_t * noise) / sqrt_alphas_cumprod_t

        return reconstructed_x_start
...
class RDM(DDPM):
    ...
    def _get_denoise_row_from_list(self, samples, desc='', force_no_decoder_quantization=False):
        ...
        for zd in tqdm(samples, desc=desc):
            denoise_row.append(self.decode_first_stage(zd.cuda(),
    ...
    def get_input(self, batch, k, return_first_stage_outputs=False, force_c_encode=False,
        ...
        with torch.no_grad():
            rep=self.pretrained_encoder.rdm_encode(batch['X'], batch['E'], batch['y'], batch['node_mask'])
            #rep=[]
            #for i in range(len(x)):
            #    rep.append(self.pretrained_encoder(x[i]))
            #rep=torch.stack(rep,0)
    ...
    def p_losses(self, x_start, cond, t, noise=None):
        ...
        return loss, loss_dict, x_noisy, model_output
    ...
    def progressive_denoising(self, cond, shape, verbose=True, callback=None, quantize_denoised=False,
        ...
        for i in iterator:
            ts = torch.full((b,), i, dtype=torch.long).cuda()
            if self.shorten_cond_schedule:
